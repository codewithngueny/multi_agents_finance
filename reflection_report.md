...
already-categorized data, and so on, making sure each agent's prompt and tool access
...
critique its own output in the same pass since it has no natural checkpoint to
...
shared state also took real design effort as early versions of the system
